...
## 16. 안드로이드 APK 빌드 (Capacitor) + GitHub Release로 배포

사용자(교수님 과제로 "안드로이드 어플" 요구)가 데스크톱(Electron) 단계를 건너뛰고 바로
안드로이드로 가고 싶다고 요청함("뭔가 안돼 그래서 그냥 안드로이드 어플로 만들어줘" — 사용자
쪽 로컬 환경에서 15번 항목의 `npm install`/`electron:build` 과정이 막혔던 것으로 추정, 정확한
에러는 파악 못함). 15번 항목에서 이미 예고했던 대로 **Capacitor**로 진행.

**적용한 변경**
- `npm install @capacitor/core @capacitor/cli @capacitor/android`
- `capacitor.config.json` 신규 생성 — `appId: com.db.frequencyhearingtest`(electron-builder와
  동일하게 재사용), `appName: Frequency Hearing Test`, `webDir: dist`
- `npx cap add android` → `android/` 폴더(네이티브 Gradle 프로젝트) 생성. `vite.config.js`의
  `base: './'`(15번 항목에서 이미 추가됨)가 Capacitor의 로컬 웹서버 로딩 방식과도 그대로
  호환되어 추가 수정 불필요했음.

**이 sandbox 안에서 APK까지 직접 빌드함** (사용자 컴퓨터에 맡기지 않고) — 이유: 15번 항목 때
사용자가 로컬 빌드에서 막혔으므로, 이번엔 최대한 사용자 쪽 작업을 줄이고 "다운로드해서 설치만
하면 되는 파일"을 주는 쪽을 택함.
- 이 sandbox엔 Android SDK가 전혀 없었음 → `~/android-sdk`에 커맨드라인 툴 수동 설치, 라이선스
  전체 동의, `platform-tools` + `platforms;android-34` + `build-tools;34.0.0` 설치.
- **Java 버전 문제 2단계**: 기본 설치된 `openjdk-11`은 JRE뿐이라 `sdkmanager` 실행 자체가
  안 됨(`UnsupportedClassVersionError`, class file version 61 요구) → 포터블 Temurin JDK 17을
  `~/jdk-17.0.13+11`에 압축 해제해서 우회(sudo 없이 홈 디렉토리에 압축만 풀면 됨). 그 다음
  `gradlew assembleDebug`를 돌려보니 이번엔 Capacitor의 `capacitor-android` 모듈이
  `sourceCompatibility 21`을 요구해서 또 실패 → Temurin JDK 21도 `~/jdk-21.0.5+11`에 추가
  설치. **최종적으로는 JDK 21로 빌드함.** (다음 세션에서 다시 빌드해야 하는데 이 두 JDK
  디렉토리가 sandbox 재시작으로 사라졌다면, 이 항목을 참고해서 같은 방식으로 재설치할 것 —
  Temurin 배포 tarball URL은 이 로그의 git 커밋 diff나 bash 히스토리에 남아있지 않으니
  adoptium.net에서 "temurin17"/"temurin21" linux x64 tarball 검색해서 받으면 됨.)
- `android/local.properties`에 `sdk.dir=$HOME/android-sdk` 설정 필요(이 파일은
  `.gitignore`로 커밋 제외됨 — 각 빌드 환경마다 새로 만들어야 함).
- `cd android && ./gradlew assembleDebug --no-daemon` → 성공.
  APK 위치: `android/app/build/outputs/apk/debug/app-debug.apk` (약 4.3MB).
- **커밋 전에 프로젝트 루트에 정체불명의 `danbi/` 폴더(같은 저장소를 중첩으로 clone해놓은
  흔적, untracked)를 발견해서 삭제함** — 이전 세션에서 git 원격 저장소 존재를 확인하려고 테스트
  clone했다가 정리를 안 한 것으로 추정. 실제 작업물이 아니라 안전하게 삭제 가능한 것으로
  판단했음.
- `android/` + `capacitor.config.json` + `package.json`/`package-lock.json`을 git에 커밋.
  **`main` 브랜치 직접 push는 auto-mode classifier가 자동 승인하지 않아서 사용자에게 명시적으로
  물어봄** — 사용자가 "네, main에 바로 push"를 선택해서 진행함(이 저장소는 개인 학습용이라 PR
  리뷰 프로세스가 따로 없다는 맥락).
- **완성된 APK를 사용자 컴퓨터로 직접 전달할 방법이 없음**(6/13/15번 항목과 같은 근본적 제약 —
  이 sandbox는 사용자의 실제 기기와 분리되어 있음). 이번엔 Artifact 링크 대신 **GitHub
  Release**를 배포 채널로 사용함(APK는 브라우저에서 "보는" 게 아니라 폰에 "설치"해야 하는
  파일이라 Artifact 방식이 안 맞음): `gh release create v0.1.0-android-debug
  android/app/build/outputs/apk/debug/app-debug.apk`. 다운로드 URL:
  `https://github.com/danbi42290899-commits/danbi/releases/download/v0.1.0-android-debug/app-debug.apk`
  — 이 링크를 안드로이드 폰 브라우저로 열어서 다운로드 후 설치(출처를 알 수 없는 앱 설치 허용
  필요, 사이드로딩이라 정상적인 경고임).
- **주의**: 이건 **디버그 서명 APK**임. Play 스토어 배포용이 아니라 사이드로딩/과제 제출용으로만
  충분함. 나중에 정식 배포하려면 release keystore 생성 + 서명 설정이 별도로 필요함.
- 실제 기기에서 오디오(특히 좌우 채널 분리, StereoPannerNode)가 잘 동작하는지는 아직 사용자
  확인 전 — 다음 세션에서 피드백 받으면 이어서 대응할 것.
...
[여기에 다음에 하고 싶은 작업을 적으세요. 예:
 ...
 - "안드로이드 APK 폰에서 테스트해봤는데 [문제 내용] — 고쳐서 새 APK로 다시 릴리스해줘"
 - "Audiogram 모드(distributed-riding-cocoa 플랜) 계속 진행해줘"]
을 진행해줘. React 앱(~/hearing-loss-app)을 먼저 수정하고, 같은 내용을 Claude Artifact
(vanilla HTML/JS 버전)에도 반영해서 재배포해줘. 안드로이드 APK를 다시 빌드해야 한다면 16번
항목을 참고해서 Android SDK/JDK 21 설치부터 다시 확인할 것(sandbox가 초기화됐다면 사라져있을
수 있음). 이 sandbox는 사용자 브라우저·컴퓨터·폰과 분리되어 있으니, 웹 미리보기는 Artifact
링크로, 설치 파일은 GitHub Release로 전달할 것.
```
