...
→ 좌표를 하나씩 순서대로 로봇팔에 보내면서 이동이 끝날 때까지 대기.
1~5번(사각형/삼각형/별/원/미소)은 전부 "파싱 + 실제 전송" 패턴이 동일하게
반복되고, 6번은 프로그램 종료입니다.

---

## 0. 파일 맨 위 주석 — 이 코드의 출처와 전제

로직은 아니지만, 이 스크립트를 어디서 어떻게 써야 하는지 알려주는 배경
정보입니다. mycobot 280 M5로 삼각형/사각형/별 같은 간단한 도형을 그리는
용도로 만들어졌고, 실행하려면 `pymycobot` 드라이버 라이브러리 설치가
필요하다는 것, 그리고 여기서 읽는 `.nc` gcode 파일들은 직접 손으로 쓰는 게
아니라 **Elephant Robotics의 "Luban"이라는 소프트웨어**로 도형을 그리면
자동 생성된다는 것을 알려줍니다. 즉 이 스크립트를 실제로 쓰려면 Luban에서
먼저 `square.nc`, `triangle.nc` 같은 파일들을 만들어서 같은 폴더에 둬야
합니다.
...
2~5번(삼각형·별·원·미소)은 파일 이름만 다를 뿐 1번(사각형)과 완전히
똑같은 "파싱 → for 루프로 한 점씩 전송" 패턴이 그대로 반복됩니다 — 즉 이
다섯 분기는 사실상 하나의 로직을 도형별로 복사-붙여넣기 한 것이라, 1번
분기 하나만 이해하면 나머지 넷은 자동으로 이해한 것과 같습니다. 6번은
`exit(0)`으로 프로그램을 바로 종료합니다.
...
if type == 1:
    ...
    coords_data = process_gcode('star.nc')
    for i in coords_data:
        mc.send_coords(i, draw_speed, 1)
        time.sleep(1)
elif type == 4:
    coords_data = process_gcode('circle.nc')
    for i in coords_data:
        mc.send_coords(i, draw_speed, 1)
        time.sleep(1)
elif type == 5:
    coords_data = process_gcode('smiley.nc')
    for i in coords_data:
        mc.send_coords(i, draw_speed, 1)
        time.sleep(1)
elif type == 6:
...
사용자가 고른 gcode 파일(Luban에서 미리 만들어 둔 `.nc`)을
`process_gcode`가 한 줄씩 읽어 X/Y/Z만 채우고 자세각은 기준값을 그대로
붙여 좌표 리스트로 바꾼 뒤 → 메인 실행부가 그 좌표들을 한 점씩
`mc.send_coords`로 전송하고 매번 1초씩 기다려 실제 움직임이 끝나길
확인하는 흐름**이며, 1~5번 분기 모두 도형 파일 이름만 다를 뿐 동일한
패턴이고 6번은 종료입니다.
